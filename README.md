...
在真正开始在Advanced部分添加功能之前, 我们先来让 MiniVim 在 Basic 中已经实现的功能更像真实的 Vim, 也就是测一些在 Basic 中没测但框架代码本身提供支持的 Corner Case.

该部分的测试点包含对`curswant`, `Tab`的处理, 状态栏的显示, 以及左侧亮蓝色的`~`的检查

要求主要如下:
- `h`/`l`/`j`/`k` 应正确维护 `curswant`. 经过短行或空行时保留期望列, 水平移动或实际编辑后更新期望列.
- Tab 作为一个字符存储, 显示时对齐到下一个 4 列制表位, 不转换为空格. `h`/`l` 和 `<BS>` 将一个 Tab 视为一个字符,
  `j`/`k` 按显示列定位. 在 Normal 模式下, 光标位于 Tab 展开区域的最后一列; 在 Insert 模式下, 若插入位置在 Tab 前,
  光标位于该区域的第一列. 插入, 删除, 拆行或合并行后应重新计算 Tab 的显示位置. 
- 在 Insert 模式下, 最下面一行的开头应显示 `-- INSERT --`, 退出该模式后清除. 进入 Command-line 模式时应正确显示命令.
- 缓冲区之外的行开头以亮蓝色的 `~` 填充(具体色号请看你的 Vim, 在这里你会需要用到一个简单的改变颜色的终端序列).
- 光标超出可见范围时, 视口应以使光标重新可见的最小距离上下或左右滚动; 光标仍可见时保持视口不变.

本部分测试点总计 0 分, 但如果没有通过, Advanced部分的其余部分不得分
...
本部分共 3 分. 这四个键的方向和 Basic 相同, 现在需要支持 count, 并正确维护 `curswant`.

|    命令     |       行为           |
| -----------| -------------------  |
| `[count]h` | 向左移动 count 个字符 |
| `[count]l` | 向右移动 count 个字符 |
| `[count]j` | 向下移动 count 行     |
| `[count]k` | 向上移动 count 行     |

没有 count 时只移动一个字符或一行. `h` 和 `l` 不跨行: 移动距离不够时就停在行首或行末, 不会绕到上一行或下一行.
空行上没有字符可移, 因此 `h` 和 `l` 都不移动光标. 一个 Tab 仍然只算一个字符.

`j` 和 `k` 按文本行移动. count 超过剩余行数时, 停在文件的第一行或最后一行; 已经到达对应边界时不再移动.
目标行上的位置由 `curswant` 决定: 尽量停在原来的显示列, 行太短就停在行末, 空行就停在该行唯一的位置.
即使这次没能到达期望列, 也不能把它忘掉, 后续移动到足够长的行时还要回到那一列. Tab 的显示列规则与 A0 相同.

#### `0`, `^`, `$`

本部分共 6 分. 这三个命令用来快速到达一行的开头或结尾.

| 命令 |                行为                          |
| --- | -------------------------------------------- |
| `0` | 移动到当前行的第一个字符, 包括行首的空格或 Tab   |
| `^` | 移动到当前行的第一个非空白字符                  |
| `$` | 移动到当前行的最后一个字符, 包括行尾的空格或 Tab |

这里的空白字符指空格和 Tab. 如果一行全是空白字符, `^` 停在该行最后一个字符上.
如果当前行为空行, 三个命令都停在该行唯一的位置.

`0` 本身就是命令, 不接受 count. 如果前面已经开始输入 count, 后面的 `0` 就是 count 中的一位数字, 例如 `20l` 中的 `0`.
`^` 会忽略 count, 所以 `3^` 与 `^` 相同.

`[count]$` 则有不同的含义: 先向下移动 count - 1 行, 再到那一行的行末. 例如 `3$` 到下面第二行的行末,
而不是在当前行重复执行三次 `$`. 如果下面没有足够多的行, 就到最后一行的行末.
有一个边界情况需要注意: 如果已经在文件最后一行, 且 count 大于 1, 则这次命令失败, 光标保持原位.

`0` 和 `^` 将 `curswant` 更新为到达位置的显示列. `$` 则将它设为“始终到行末”, 即前文所说的极大值;
上面最后一行无法下移的情况也会设置这个值. 此后只要连续使用 `j` 或 `k`, 无论目标行多长, 都应停在目标行的行末.

#### `gg`, `G`

本部分共 2 分. 这两个命令直接跳到指定行. 行号从 1 开始, 带 count 时表示行号, 不是移动次数.

| 命令        | 目标行          |
| ----------- | -------------- |
| `gg`        | 文件第一行      |
| `G`         | 文件最后一行    |
| `[count]gg` | 文件第 count 行 |
| `[count]G`  | 文件第 count 行 |

例如, `5gg` 和 `5G` 都到第 5 行. `G` 与 `1G` 不同, 后者到第一行. 如果指定的行号超过文件总行数, 就到最后一行.

到达目标行后, 光标停在第一个非空白字符上. 如果那一行全是空格和 Tab, 就停在最后一个字符上;
如果是空行, 就停在该行唯一的位置. 即使目标行就是当前行, 也要按这条规则重新定位, 不能直接认为没有事可做.
完成后将 `curswant` 更新为光标所在的显示列.

#### `w`, `b`, `e`, `ge`

本部分共 9 分. 这四个命令按 word 移动. 这里的 word 不完全是自然语言中的“单词”, 我们先约定它如何划分:

- 连续的英文字母, 数字和下划线组成一个 word.
- 连续的其他非空白字符组成一个 word.
- 空格, Tab 和换行将它们分开. 一个 word 不会跨越两行.

例如, `hello += world_2;` 中有四个 word: `hello`, `+=`, `world_2`, `;`.
`foo.bar` 中也有三个 word: `foo`, `.`, `bar`, 虽然它们之间没有空格.
本项目只考虑可打印 ASCII 字符和 Tab, 不要求处理中文等字符的 word 划分.

我们还需要区分两种情况: **空行算一个 word**, 它的开头和结尾都是该行唯一的位置;
**只含空格或 Tab 的行没有 word**, 移动时会跳过这些空白.

四个命令的具体行为如下:

- `w`: 向后到下一个 word 的开头. 如果当前在一个 word 上, 先越过这个 word; 如果当前在空白上,
  则跳过这些空白. 因此, 即使已经在 word 的开头, 也要去下一个, 不能停在原地.
- `b`: 向前到 word 的开头. 如果当前在一个 word 内部但不在开头, 就回到这个 word 的开头;
  如果已经在开头, 则去上一个 word 的开头. 当前在空白上时, 跳过前面的空白, 去前一个 word 的开头.
- `e`: 向后到 word 的结尾. 如果当前在一个 word 上但还没到结尾, 就到这个 word 的最后一个字符;
  如果已经在结尾, 则去下一个 word 的结尾. 当前在空白上时, 跳过后面的空白, 去后一个 word 的结尾.
- `ge`: 向前到上一个 word 的结尾. 当前在 word 上时, 无论是在开头, 中间还是结尾, 都去上一个 word 的最后一个字符;
  当前在空白上时, 则去前一个 word 的最后一个字符. 例如在 `abc def` 的 `e` 上按 `ge`, 应到 `abc` 的 `c`, 而不是 `def` 的 `d`.

这些命令可以跨行. `w`, `b`, `ge` 都会把遇到的空行当作一个 word 停下来, 连续空行也要分别计算.
**`e` 是例外: 它跳过空行**, 继续找后面的非空 word. 只包含空白字符的行对四个命令来说都不是 word.

它们都支持 count, 行为相当于连续执行相应命令 count 次. 例如 `3ge` 相当于连续执行三次 `ge`,
count 写在整个命令前面, 不是写成 `g3e`.

如果向后已经找不到目标, `w` 和 `e` 最多移到文件最后一行的最后一个字符; 如果向前已经找不到目标,
`b` 和 `ge` 最多移到文件第一行的第一个字符. 对应行为空行时, 就停在该行唯一的位置.
这条规则也适用于文件开头或结尾全是空白的情况: 不会退回最后找到的 word, 而是移动到文件边界.
已经在对应边界时不再移动, count 再大也一样. 每条命令完成后, 将 `curswant` 更新为终点的显示列.

#### `W`, `B`, `E`, `gE`

本部分共 5 分. 大写的 WORD 使用更简单的划分方式: **一段连续的非空白字符就是一个 WORD**, 不再区分字母, 数字和标点.
空格, Tab 和换行才会将它们分开. 例如 `foo.bar += baz_1;` 中只有三个 WORD: `foo.bar`, `+=`, `baz_1;`.

| 命令 | 行为 |
| --- | --- |
| `W` | 向后到下一个 WORD 的开头 |
| `B` | 向前到 WORD 的开头; 已经在开头时去上一个 WORD |
| `E` | 向后到 WORD 的结尾; 已经在结尾时去下一个 WORD |
| `gE` | 向前到上一个 WORD 的结尾 |

除了把 word 换成 WORD, 其余规则分别与 `w`, `b`, `e`, `ge` 相同: 当前在空白上的处理方式不变,
空行算一个 WORD, 只含空白的行没有 WORD; `W`, `B`, `gE` 会停在空行, `E` 跳过空行.
它们同样可以跨行, 支持 count, 并按上一节的规则处理文件边界和 `curswant`. 例如 `2gE` 相当于连续执行两次 `gE`.
